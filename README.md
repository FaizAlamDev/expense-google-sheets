...
The History UI (`client/src/components/HistoryPage.tsx` / the matching mobile screen) shows **one month per page** with a `‹ Prev  July 2026  Next ›` month navigator, a header card breaking down **Dated / Adjustments / Grand Total**, an **Adjustments** section (add/edit/delete signed rows), and the month's day rows; clicking a day opens an animated **modal** with that day's full card and inline add/edit/delete (`DateGroupCard`). A date search shows only that single date's row (empty dates show "No expenses on YYYY-MM-DD"). Each mutation re-fetches so the month stays consistent. The 10-slot-per-day invariant is still enforced server-side on `POST /api/expenses`. Edits and deletes also trigger a **best-effort background rewrite** of the affected date's row in Google Sheets (`syncDateGroupToGoogleSheets`), serialized per date. Google Sheets month-Total rows are **not yet automated** (planned: `docs/monthly-totals-plan.md`).
