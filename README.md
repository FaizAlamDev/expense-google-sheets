...
**Monthly totals on the sheet (automated).** When the first expense of a brand-new month lands on the sheet, the previous month(s) are **finalized**: a month-Total row is appended two blank rows below that month's last day row with `A="."`, `K="Total"` (bold + light-highlighted), `L==SUM(L{firstDay}:L{lastDay})+M{row}`, and `M=<net adjustments>`. Month-Total rows are **never rewritten** — if a Total row already occupies the computed position (including the manual ones you wrote before this feature), it is left untouched. There is **no mid-sheet row insertion**: everything is a pure append. Add/edit/delete of monthly adjustments fires `syncMonthTotalToGoogleSheets()`, which re-syncs **only the `M` cell** of the month's existing Total row (no-op if the month has no Total row yet).
...
- `GET /api/expenses/months?limit=&offset=` — month-wise history, newest first. Returns `{ total, months: [{ month, dayTotal, adjustmentTotal, total, days: [{date,count,total}], adjustments }] }`. `total` (month) = `dayTotal + adjustmentTotal`; the month set is the UNION of expense-months and adjustment-months.
...
- Monthly adjustments (signed amounts not tied to any date, e.g. round-offs): `POST /api/expenses/adjustments` `{month: 'YYYY-MM', amount, label?}`; `PUT /api/expenses/adjustments/:id` `{amount, label?}`; `DELETE /api/expenses/adjustments/:id`. `amount` must be a non-zero number (negative subtracts); `label` is free text.

The History UI (`client/src/components/HistoryPage.tsx` / the matching mobile screen) shows **one month per page** with a `‹ Prev  July 2026  Next ›` month navigator, a header card breaking down **Dated / Adjustments / Grand Total**, an **Adjustments** section (add/edit/delete signed rows), and the month's day rows; clicking a day opens an animated **modal** with that day's full card and inline add/edit/delete (`DateGroupCard`). A date search shows only that single date's row (empty dates show "No expenses on YYYY-MM-DD"). Each mutation re-fetches so the month stays consistent. The 10-slot-per-day invariant is still enforced server-side on `POST /api/expenses`. Edits and deletes also trigger a **best-effort background rewrite** of the affected date's row in Google Sheets (`syncDateGroupToGoogleSheets`), serialized per date. Google Sheets month-Total rows are automated too — see "Monthly totals on the sheet" in the mirror section above.
